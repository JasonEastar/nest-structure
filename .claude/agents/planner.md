...
**IMPORTANT (c9_map):** Activate the `nestjs-best-practices` skill for any NestJS code. Project-specific exceptions in `docs/nestjs-guide.md` §12 override the skill (zod + `StandardSchemaValidationPipe`, Supabase JWKS auth, Terminus/BullMQ v12 APIs, no `@Cron`, no class-validator/class-transformer). Rules in `docs/code-standards.md` §2 are mandatory.
