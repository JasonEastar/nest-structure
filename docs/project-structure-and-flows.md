...
sequenceDiagram
  ...
  T->>T: Redis INCR theo ip
  T-->>M: 429 + Retry-After
  T->>A: ok
  A->>A: jose.jwtVerify(JWKS) · ensureProfile(sub) (Redis { status } 1h → INSERT ON CONFLICT)
  A-->>M: 401 UNAUTHENTICATED · 403 ACCOUNT_BLOCKED
  A->>P: req.user
  P->>P: @RequirePermission ↔ c9:perms:{id} 300s
  P-->>M: 403 FORBIDDEN
  P->>V: ok
...
sequenceDiagram
  ...
  API->>RD: GET c9:v1:profile:{sub} ({ status })
  RD-->>API: miss
