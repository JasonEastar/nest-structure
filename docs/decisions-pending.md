...
| 40 | Bull Board mở bằng trình duyệt qua `?access_token=` → token lộ trong access log/history; UI gọi API không kèm query → 401 | Bảo mật admin tool | ✅ 2026-09-17: query chỉ dùng lần mở trang, server đặt cookie HttpOnly path /admin/queues 1 giờ cho các lần gọi sau. Prod: bỏ query param, chỉ Bearer qua reverse proxy có auth. 2026-09-21: gỡ Bull Board (chưa có queue), thêm lại cùng queue đầu tiên (bước 9) | ☐ |
| 41 | Admin web đăng nhập username + password (không Google): tự làm auth trong backend hay vẫn Supabase? | Bảo mật, 2 luồng auth | Vẫn **Supabase email + password** (Enable Email provider, tắt sign-up công khai, admin tạo tài khoản bằng `admin.createUser`); backend giữ nguyên chỉ verify JWT. `profiles.username` đặt một lần khi tạo tài khoản, không sửa qua `PATCH /me` (2026-09-20). Chỉ tự làm password nếu bắt buộc tách khỏi Supabase | ✅ 2026-09-20: `POST /admin/users` gọi `admin.createUser` (email_confirm, `must_change_password`), admin đầu tiên qua `scripts/dev-token.mjs` (dev) hoặc gán role bằng SQL (server). Đã kiểm với Supabase thật (`supabase-real.spec`). Còn mở: bắt MFA (`aal2`) cho `/admin/*` khi có > 2 admin |
