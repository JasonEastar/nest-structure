...
- `modules/<x>/`: file chính ở gốc, tên bắt đầu bằng `<x>.` hoặc `<x>-`: `<x>.module|controller|service|repository|constants|jobs.ts`; `<x>-<sub>.controller.ts` cho nhóm route phụ (`<x>-admin` cần permission, `<x>-public` không cần đăng nhập, path `public/<x>`); `<x>-geo.repository.ts` cho SQL PostGIS. Chỉ 2 thư mục con: `dto/` và `schema/`. Không tạo `controllers/ services/ repositories/`. **Mẫu chuẩn: `modules/location/`** — module mới copy y hệt.
