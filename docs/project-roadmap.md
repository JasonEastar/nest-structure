...
| 3. Nền dữ liệu | Drizzle + Postgres/PostGIS riêng, migration đầu (postgis, unaccent, pg_trgm), `common/database/drizzle.ts`, `schema/identity.schema.ts` | `drizzle-kit migrate` chạy, `/health/ready` → 200 | MVP | ✅ 2026-09-16 |
| 4. Cross-cutting | `AllExceptionsFilter`, `AppException`, `ErrorCodes`, zod pipe, pino + `requestId`, i18n, Swagger 2 document | Lỗi trả đúng shape `{error:{code,params,requestId}}`, `/docs/app` mở được, `openapi.json` export | MVP | ✅ 2026-09-16 |
| 5. Redis | `common/redis/cache.ts` (CacheService), `common/redis/throttler.guard.ts`, `common/redis/queue.ts` + `pin.jobs.ts` | Rate limit đếm chung qua 2 instance; queue ping job chạy 1 lần | MVP | ✅ 2026-09-16 |
| 6. Supabase Auth + RBAC | Verify JWT Google (JWKS), `AuthGuard` + `@Public()`, upsert `profiles` phía app + role `user`, bảng RBAC + seed, `PermissionGuard`, admin API gán role, `PhoneVerifiedGuard` (gđ 2) | E2E: token Supabase local → `/me` trả profile; token giả → 401 | MVP | ✅ 2026-09-17 |
