...
| 6. Supabase Auth + RBAC | Verify JWT Google (JWKS), `AuthGuard` + `@Public()`, upsert `profiles` phía app + role `user`, bảng RBAC + seed, `PermissionGuard`, admin API gán role (guard SĐT đã xác minh: gđ 2), `PATCH /me` sửa hồ sơ, `/admin/users` tạo tài khoản email + mật khẩu / danh sách / khoá (`profiles.status`, migration 0005), `scripts/dev-token.mjs` (user + role + token cho dev); permission tách theo hành động (migration 0006); role/permission/nhóm là dữ liệu DB, admin CRUD (0009–0012); `GET /public/configs` enum + nhãn (2026-09-20) | E2E: token Supabase local → `/me` trả profile; token giả → 401 | MVP | ✅ 2026-09-17 |
