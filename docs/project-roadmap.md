...
| 5. Redis | `common/redis.ts` (2 connection + CacheService), `common/throttler.guard.ts`, `common/queue.ts` + `pin.jobs.ts` | Rate limit đếm chung qua 2 instance; queue ping job chạy 1 lần | MVP | ✅ 2026-09-16 |
| 6. Supabase Auth + RBAC | Verify JWT Google (JWKS), `AuthGuard` + `@Public()`, upsert `profiles` phía app + role `user`, bảng RBAC + seed, `PermissionGuard`, admin API gán role, `PhoneVerifiedGuard` (gđ 2) | E2E: token Supabase local → `/me` trả profile; token giả → 401 | MVP | ✅ code 2026-09-17 · chờ bật JWT keys |
