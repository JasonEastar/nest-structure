...
| 6. Supabase Auth + RBAC | Verify JWT Google (JWKS), `AuthGuard` + `@Public()`, upsert `profiles` phía app + role `user`, bảng RBAC + seed, `PermissionGuard`, admin API gán role, `PhoneVerifiedGuard` (gđ 2) | E2E: token Supabase local → `/me` trả profile; token giả → 401 | MVP | ☐ |
