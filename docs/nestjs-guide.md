...
{ provide: APP_GUARD,       useClass: AppThrottlerGuard }   // 1. rate limit theo IP, trước khi tốn CPU verify JWT
{ provide: APP_GUARD,       useClass: AuthGuard }           // 2. Bearer → JWKS → req.user; @Public() bỏ qua
{ provide: APP_GUARD,       useClass: PermissionGuard }     // 3. @RequirePermission('pin:create')
...
- Sau `createDocument`: duyệt `paths`, tra `Reflector` (`@Public`, `@RequirePermission`) qua `DiscoveryService.getControllers()` để ghi "Quyền cần có"/"Không cần đăng nhập" vào description (config/openapi.ts `annotateAccess`).
